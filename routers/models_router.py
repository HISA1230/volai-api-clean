...
def rename_model(
    ...
    src = Path("models") / old_name
    dst = Path("models") / new_name
    if not src.exists():
    ...
    shap_old = Path("models") / (old_name.replace(".pkl", "_shap_summary.csv"))
    shap_new = Path("models") / (new_name.replace(".pkl", "_shap_summary.csv"))
    if shap_old.exists():
        try:
...
def delete_model(
    ...
    p = Path(model_path)
    if not p.exists():
        p = Path("models") / Path(model_path).name
    if not p.exists():
